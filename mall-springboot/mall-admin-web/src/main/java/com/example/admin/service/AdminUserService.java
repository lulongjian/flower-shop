...
public class AdminUserService {
    ...
    public void deleteUserById(Integer id) {
        ...
        userDO.setId(id);
        if (Objects.equals("1", userDO.getPermission())) {
    ...
    public UserDO getUserIdByUserName(UserForm userForm) {
        ...
        example.createCriteria().orEqualTo("sno", userForm.getSno())
                .orEqualTo("email", userForm.getEmail());
    ...
    public JsonModel selectUserList() {
        UserDO userDO = new UserDO();
        userDO.setRole("普通用户");
        List<UserDO> list = userMapper.select(userDO);

        JsonModel jsonModel = new JsonModel();
        jsonModel.setCount((long) list.size());
