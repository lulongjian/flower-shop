  ...
  <div>
    <avue-crud :option="option" :search.sync="search" :page.sync="page" :data="tableListData" :table-loading="loading"
      ...
      <template slot-scope="" slot="contentForm">
        <avue-ueditor v-model="notice.content" :options="ueditorOptions">
...
import { getNoticeListRequest, addNoticeRequestRequest, updateNoticeRequestRequest, delNoticeRequestRequest } from "@/api/notice"
export default {
  name: 'NoticeManageVue',
  data () {
    return {
      userList: [],
      productList: [],
  ...
  methods: {
    ...
    getListData () {
      ...
      getNoticeListRequest(params).then(response => {
    ...
    handleRowSave (row, done, loading) {
      ...
      addNoticeRequestRequest(params).then(response => {
    ...
    rowUpdate (row, index, done, loading) {
      ...
      updateNoticeRequestRequest(params).then(response => {
        this.$message.success(response.msg)
        this.getListData()
        done(row)
      })
    ...
    rowDel (row, done, loading) {
      this.$confirm('确认操作？')
        .then(_ => {
          ...
          delNoticeRequestRequest(params).then(response => {
            this.$message.success(response.msg)
            this.getListData()
            done(row)
          }).finally(() => {
